...
{
    // Fix: Implement proper double SHA256 for Bitcoin address checksum
    // Bitcoin uses SHA256(SHA256(payload)) for checksum calculation

    // Convert uint32_t arrays back to byte arrays for proper double SHA256
    uint8_t payload0[21], payload1[21], payload2[21], payload3[21];

    // Extract the 21-byte payloads from the uint32_t arrays
    // The CHECKSUM macro packs the 21-byte payload into the first 6 uint32_t values
    for(int i = 0; i < 5; i++) {
        payload0[i*4] = (i0[i] >> 24) & 0xFF;
        payload0[i*4+1] = (i0[i] >> 16) & 0xFF;
        payload0[i*4+2] = (i0[i] >> 8) & 0xFF;
        payload0[i*4+3] = i0[i] & 0xFF;

        payload1[i*4] = (i1[i] >> 24) & 0xFF;
        payload1[i*4+1] = (i1[i] >> 16) & 0xFF;
        payload1[i*4+2] = (i1[i] >> 8) & 0xFF;
        payload1[i*4+3] = i1[i] & 0xFF;

        payload2[i*4] = (i2[i] >> 24) & 0xFF;
        payload2[i*4+1] = (i2[i] >> 16) & 0xFF;
        payload2[i*4+2] = (i2[i] >> 8) & 0xFF;
        payload2[i*4+3] = i2[i] & 0xFF;

        payload3[i*4] = (i3[i] >> 24) & 0xFF;
        payload3[i*4+1] = (i3[i] >> 16) & 0xFF;
        payload3[i*4+2] = (i3[i] >> 8) & 0xFF;
        payload3[i*4+3] = i3[i] & 0xFF;
    }

    // Handle the last byte (from i0[5] which contains the last byte + padding)
    payload0[20] = (i0[5] >> 24) & 0xFF;
    payload1[20] = (i1[5] >> 24) & 0xFF;
    payload2[20] = (i2[5] >> 24) & 0xFF;
    payload3[20] = (i3[5] >> 24) & 0xFF;

    // Calculate proper double SHA256 checksums
    sha256_checksum(payload0, 21, d0);
    sha256_checksum(payload1, 21, d1);
    sha256_checksum(payload2, 21, d2);
    sha256_checksum(payload3, 21, d3);
