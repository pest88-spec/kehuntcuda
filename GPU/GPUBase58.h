...
__device__ __noinline__ void _GetAddress(int type, uint32_t *hash, char *b58Add)
{
    ...
    // Debug: Print payload before checksum calculation
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        printf("GPU payload: ");
        for(int i = 0; i < 21; i++) printf("%02x", A[i]);
        printf("\n");
    }
...
#pragma unroll 8
    ...
    // Fix: Reverse the byte order to match expected checksum
    // Expected: 18C46811, GPU generates: CDBA5AEE (which is byte-reversed)
    A[21] = ((uint8_t *)s)[0];  // Reverse the original order
    A[22] = ((uint8_t *)s)[1];
    A[23] = ((uint8_t *)s)[2];
    A[24] = ((uint8_t *)s)[3];
